...
function getProperty(propName, properties, opt) {
  try {
    ...
    var defaultFallback = (opt.lt || defaultLt) + propName + (opt.gt || defaultGt);
    var tFallback = typeof opt.fallback;
    if (tFallback == 'function') {
      var fallback = opt.fallback(propName);
      if (typeof fallback == 'string') {
        return fallback;
      } else {
        return defaultFallback;
      }
    } else if (tFallback == 'string') {
      return opt.fallback;
    } else {
      return defaultFallback;
    }
...
module.exports.replaceProperties = replaceProperties;
